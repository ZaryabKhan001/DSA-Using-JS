...
// Choices are +, - and * or no operator (to form a multi-digit number).
...
var addOperators = function (num, target) {
    ...
    const backTrack = (start, expression) => {
        ...
        for (let i = start; i < num.length; i++) {
            ...
            // First number cannot have an operator before it
            if (start === 0) {
                backTrack(i + 1, digit);
            } else {
...
var addOperators = function (num, target) {
    ...
    const map = {
        '+': (evaluatedValue, prev, digit) => evaluatedValue + digit,
        '-': (evaluatedValue, prev, digit) => evaluatedValue - digit,
        '*': (evaluatedValue, prev, digit) => evaluatedValue - prev + prev * digit,
    };

    const backTrack = (start, expression, evaluatedValue, prev) => {
        if (start === num.length) {
        ...
        for (let i = start; i < num.length; i++) {
            ...
            let digit = Number(num.substring(start, i + 1));

            if (start === 0) {
                backTrack(i + 1, digit.toString(), digit, digit);
            } else {
                for (let operator of operators) {
                    backTrack(
                        i + 1,
                        expression + operator + digit,
                        map[operator](evaluatedValue, prev, digit),
                        operator === '-' ? -digit : operator === '*' ? prev * digit : digit
                    );
                }
            }
        }
