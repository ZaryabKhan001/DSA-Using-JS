...
var combinationSum2 = function (candidates, target) {
    ...
    const backTrack = (start, currentSum, subset) => {
        //* base case
        ...
        for (let i = start; i < candidates.length; i = i + 1) {
            ...
            //* include candidate into subset
            subset.push(candidate);

            //* explore combinations with the current element included
            backTrack(i + 1, currentSum + candidate, subset);

            //* Backtrack: remove the current candidate
            subset.pop();
...
//? Space Complexity = O(2^n * n) (output) + O(n) (stack)
