...
class Solution {
  merge(head1, head2) {
    ...
    if (head1.data < head2.data) {
      result = head1;
      head1.bottom = this.merge(head1.bottom, head2);
    } else {
      result = head2;
      head2.bottom = this.merge(head1, head2.bottom);
    }
  ...
  flatten(root) {
    ...
    const newRoot = this.flatten(root.next);
    return this.merge(root, newRoot);
  }
}

//? Time Complexity: O(N*M) // N = number of nodes in the main list, M = number of nodes in the sub-linked list
