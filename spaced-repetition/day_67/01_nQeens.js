...
var solveNQueens = function (n) {
    ...
    const backTrack = (rowIndex) => {
        //* base case
        if (rowIndex === n) {
            const answer = chessBoard.map((row) => row.join(''));
            result.push([...answer]);
            return;
        }
...
//* just here we are expected to return answer in col wise placing order.

// Input: n = 4
// Output: [[2, 4, 1, 3], [3, 1, 4, 2]]';

//* Everything remains same as above, just we need to return the answer in col wise placing order, so we will traverse the chessBoard column wise and check for all rows. 
